...
const ProjectCard = ({ project, style, className = '' }) => {
  return (
    <div style={style} key={project.id} className={`dark:bg-[#1a1a1a] bg-gray-100 dark:border-[#2a2a2a] border-gray-200 rounded-lg p-4 sm:p-6 flex flex-col justify-between hover:shadow-lg hover:shadow-pink-500/10 transition-shadow duration-300 ${className}`}>
      <div>
      ...
      <div className="flex justify-end gap-4 mt-4">
        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label={`Ver el código de ${project.title} en GitHub`} className="text-gray-400 hover:text-pink-500 transition-all duration-300 hover:scale-110">
          <GitIcon className="fill-current w-6 h-6" />
        </a>
        <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" aria-label={`Ver ${project.title} en vivo`} className="text-gray-400 hover:text-pink-500 transition-all duration-300 hover:scale-110">
